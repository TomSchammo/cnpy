...
template <typename T>
concept SignedInteger = std::is_same_v<T, int> || std::is_same_v<T, char> ||
...
template <typename T>
concept ComplexNumber = std::is_same_v<T, std::complex<float>> ||
...
template <typename T> consteval char map_type() {
  if constexpr (FloatingPoint<T>)
    ...
    return 'i';
  else if constexpr (UnsignedInteger<T>)
    return 'u';
  else if constexpr (std::is_same_v<T, bool>)
    return 'b';
  else if constexpr (ComplexNumber<T>)
    return 'c';
  else
    return '?';
}
